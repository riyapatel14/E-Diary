...
def searchNotes(request):
    ...
    try:
        if time_frame == 'today':
            notes = Notes.objects.filter(Q(noteTitle__icontains=sd), created_at__gte=datetime.now().replace(hour=0, minute=0, second=0, microsecond=0))
        elif time_frame == 'week':
            notes = Notes.objects.filter(Q(noteTitle__icontains=sd), created_at__gte=datetime.now() - timedelta(days=7))
        elif time_frame == 'month':
            notes = Notes.objects.filter(Q(noteTitle__icontains=sd), created_at__gte=datetime.now() - timedelta(days=30))
        else:
